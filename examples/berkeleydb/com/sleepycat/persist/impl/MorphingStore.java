...
public class Store {
    ...
    public synchronized <PK,E> PrimaryIndex<PK,E>
		...
		*/
	    ...
	    String seqName;
	    <PK>[f] for ( @PrimaryKey(sequence=seqName) PK f : E.fields ) {|
	    MorphingPersistKeyBinding<PK> keyBinding = getKeyBinding(primaryKeyClass);

            /* Create bindings. */
            MorphingPersistEntityBinding<E> entityBinding =
		new MorphingPersistEntityBinding<E>(rawAccess);
		...
		DbCompat.getThreadTransaction(env) == null) {
            ...
            try {
                ...
                /* Create index object. */
                priIndex = new MorphingPrimaryIndex<PK,E>
    ...
    /**
     * Cannot replace because EntityMetadata contains other useful
     * information such as database connection information.
     **/
    ...
    private MorphingPersistKeyBinding<K> getKeyBinding(Class<K> keyClass) {
        MorphingPersistKeyBinding<K> binding = keyBindingMap.get(keyClass);
        if (binding == null) {
            binding = new MorphingPersistKeyBinding<K>(rawAccess);
            keyBindingMap.put(keyClass, binding);
        }
        return binding;
    }
